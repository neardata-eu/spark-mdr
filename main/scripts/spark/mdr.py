...
# HDFS path
HDFSPATH = "hdfs://"+MASTER+":9000/input/"
HDFSOUTPATH = "hdfs://"+MASTER+":9000/output/"
...
labels = spark.read.options(header = True, delimiter = " ").csv(HDFSPATH + "labels.sample")
...
files = sc.textFile(HDFSPATH + "listoffiles.txt").collect()
...
for f1 in files:
    ...
    # Read file 1
    rdd1 = spark.sparkContext.textFile(HDFSPATH + f1)
    ...
    for f2 in files:
        ...
        rdd2 = spark.sparkContext.textFile(HDFSPATH + f2)
        ...
        #print("Saving data...")
        mdrerror.saveAsTextFile(HDFSOUTPATH + outfolder)
        #print("Data saved.")
        #print("")
...
with open(LOGPATH + "/test_c" + str(CORES) + "_w" + str(WORKERS) + ".log", 'a') as outfile:
