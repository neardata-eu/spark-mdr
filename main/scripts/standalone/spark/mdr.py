...
labels = spark.read.options(header = True, delimiter = " ").csv("file:///" + DATAPATH + "labels.sample")
...
files = sc.textFile("file:///" + DATAPATH + "listoffiles.txt").collect()
...
for f1 in files:
    ...
    # Read file 1
    rdd1 = spark.sparkContext.textFile("file:///" + DATAPATH + "samples/" + f1)
    ...
    for f2 in files:
        ...
        rdd2 = spark.sparkContext.textFile("file:///" + DATAPATH + "samples/" + f2)
...
with open(LOGPATH + "/test_st_c" + str(CORES) + "_w" + str(WORKERS) + ".log", 'a') as outfile:
